...
import { exportDocument, validateDocument } from "@/domain/persistence/save-service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ taskId: string }> }) {
  ...
  if (!validation.valid) return ok({ valid: false, errors: validation.errors }, 422);

  const files = exportDocument(document);
  const zip = new JSZip();
  ...
  const fileBaseName = body.fileBaseName ?? document.sourceName.replace(/\.(json|properties)$/, "");
