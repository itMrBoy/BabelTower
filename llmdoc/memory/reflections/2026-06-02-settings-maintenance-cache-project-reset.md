...
- 当前环境缺少可用的 `node_modules/.bin`，`npm run typecheck` 和 `npm test` 无法找到 `tsc`/`vitest`。
- `npm ci` 被 Prisma Windows query engine DLL 占用阻塞；依赖恢复后应补跑 `npm run typecheck` 和 `npm test -- settings-maintenance`。
