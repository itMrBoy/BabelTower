...
| `/export` | `src/app/export/page.tsx` | 导出页面。双文件下载（源文件+译文）、导出前验证、中文错误提示 |
...
- `Sidebar`：5 个导航项（上传、冲突、字典、快照、导出），品牌图标 `babeltower-icon.svg`
- `TopBar`：页面标题 + 语言对徽章 `zh-CN -> en-US`
- `MessageProvider`：Toast 通知系统（Context，成功 3 秒，错误/警告 6 秒）
...
- **包管理器**：pnpm（强约束，详见 `reference/ci-and-tooling.md`）
- **Docker**：两阶段构建（builder -> runner），基于 node:24-alpine，统一用 `corepack` + pnpm 安装；runner（standalone）镜像内无 pnpm，仅全局装 prisma（容器内迁移用裸 `prisma db push`），详见 `reference/ci-and-tooling.md`
- **CI**：GitHub Actions，6 个并行 job（lint, typecheck, prisma-validate, openapi-validate, test, build），统一基于 pnpm；工具链与版本锁定详见 `reference/ci-and-tooling.md`
...
| 模块级内存变量 `workspaceStateCache` | `page.tsx` | 首页工作区状态在 SPA 内存中缓存，刷新浏览器即丢失（非持久化） |
| 内存变量 + `Set` 监听器 | `current-task.ts` | 跨页面当前任务同步；另含未落库编辑 `CurrentTaskDraftBuffer`，供导出页导出前补暂存 |
